...
//	    }
  ...
  private:
    ...
    void displayTree( AvlNode *t, int depth, ostream & out )
    {
        const int SHIFT = 4 ;
        if( t != nullptr )
        {
	...
	int contains_search_depth( const Comparable &x, AvlNode *t) {
		if (contains(x)){
			return depth(x,t);
		}
		AvlNode* temp;
	...
	const int contains_search_depth(const Comparable &x, AvlNode *t) const {
		if (contains(x)){
				return depth(x,root);
			}
			AvlNode* temp;
...
//        }
//        }
        bool hasTwoChild(AvlNode *node1){
        	return (node1->left!=nullptr && node1->right != nullptr);
        }

        bool isLeaf(AvlNode *node1){
        	return (node1->left==nullptr && node1->right == nullptr);
        }

        bool hasOneLeftChild(AvlNode *node1){
        	return (node1->left!=nullptr && node1->right == nullptr);
        }

        bool hasOneRightChild(AvlNode *node1){
        	return (node1->left==nullptr && node1->right != nullptr);
        }


        int depth( const Comparable & x,AvlNode *t) { //assume x is in the tree
        	while (t->element != x){
        		if (x > t->element){
